                        ...
                        <table class="table table-hover" id="dataTable" width="100%" cellspacing="0">
                            ...
                            <tbody >
                                ...
                                @foreach ($posts as $post)
                                    <tr>
                                        <td><input type="checkbox" {{ $post->hidden === 1 ? 'checked' : "" }}></td>
                                        <td>{{ $post->id }}</td>
                                        ...
                                        <td>
                                            @if($post->hidden == 1)
                                                <a class="btn btn-danger btn-sm" href="{{ url('admin/hidden/'.$post->id) }}">
                                                    <i class="fa fa-eye-slash" aria-hidden="true"></i>
                                                </a>
                                            @else
                                                <a class="btn btn-danger btn-sm" href="{{ url('admin/unhidden/'.$post->id) }}">
                                                    <i class="fa fa-eye" aria-hidden="true"></i>
                                                </a>
                                            @endif
                                        </td>
                                        <td><a href="{{ route('admin.blog.show', ['blog' => $post->id]) }}"
                                                class="btn btn-warning btn-sm">Edit</a></td>

                                       <td><a class="btn btn-danger btn-sm" href="{{ url('admin/delete/'.$post->id) }}">Delete</a></td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
